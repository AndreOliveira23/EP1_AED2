...
void buscaArvoreGeradoraMaxima( Aresta* ArvoreGeradoraMaxima, int vertice_origem, int vertice_destino, float menorPeso_backTracking){
	...
	printf("Antes insertionSort: Arvore[0] %d -> %d \n",ArvoreGeradoraMaxima[0].origem,ArvoreGeradoraMaxima[0].destino);
	    	...
	    	printf("(tamanho arvore_global =%d)\n",tamanho_arvore);	
	    				
	    // Classificar as arestas
	    insertionSortPorOrigemAresta(ArvoreGeradoraMaxima,tamanho_arvore);	
		printf("Apos insertonSort: Arvore[0] %d -> %d: \n",ArvoreGeradoraMaxima[0].origem,ArvoreGeradoraMaxima[0].destino);
		
    Aresta atual = ArvoreGeradoraMaxima[0];	
	printf("Atual: %d -> %d : %.1f\n Vertice_destino: %d\n",atual.origem,atual.destino,atual.peso,vertice_destino);
	
	printf("Percorrendo até achar a origem correta\n");	
	...
	//Percorrendo até achar o destino certo
	while(atual.destino != vertice_destino){
		printf("Atual antes of if: %d -> %d : %.1f\n",atual.origem,atual.destino,atual.peso);		
		if(i == tamanho_arvore-1	){
			if(atual.destino == vertice_destino){
					printf("Atual no meio destino: %d -> %d : %.1f		\n",atual.origem,atual.destino,atual.peso);		
					printf("?");
					return;
			}else{ //Backtracking
					printf("Não existe vértice com destino no vértice solicitado...fazendo backtracking....\n");
	...
	printf("Atual após  percorrer destino: %d -> %d : %.1f		\n",atual.origem,atual.destino,atual.peso);	
	printf("Vertice destino:  %d\n",vertice_destino);	

	//Assumindo que achou o lugar correto
	if (atual.peso < menorPeso.peso){
		menorPeso = atual;
	}
	printf("Aresta certa::Atual final: %d -> %d : %.1f		\n",atual.origem,atual.destino,atual.peso);			
